...
--toDigits 0 = []
{--toDigits n = if n > 0
                then (toDigits (n `quot` 10))++(n `mod` 10):[]  
                else if n == 0 then [0] else undefined
...
doubleSecond' xs = zipWith (+) (cycle [0,2]) xs
...
sumDigits [] = 0
sumDigits (x:xs) = sumDigits xs + sumDigitsIndividualEle x
                    where 
                       sumDigitsIndividualEle 0 = 0
                       sumDigitsIndividualEle x = sumDigitsIndividualEle (x `div` 10) + x `mod` 10
